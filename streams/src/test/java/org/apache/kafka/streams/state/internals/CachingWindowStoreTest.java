...
public class CachingWindowStoreTest {
    ...
    public void setUp() {
        ...
        topic = "topic";
        context = new InternalProcessorContextMockBuilder().stateDir(TestUtils.tempDirectory()).cache(cache).build();
    ...
    public void shouldFetchAllWithinTimestampRange() {
        ...
        for (int i = 0; i < array.length; i++) {
            context.setRecordContext(new ProcessorRecordContext(i, context.offset(), context.partition(), context.topic(), context.headers()));
